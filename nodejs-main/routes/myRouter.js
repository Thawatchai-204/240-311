...
router.post('/buy-book', (req, res) => {
    ...
    // Process the form data and complete the purchase here
...
router.get('/manage', (req, res) => {
    if (req.session.login) {  //ดูว่า login มีค่า true รึปล่าว
        Product.find().exec((err, doc) => {
            res.render('manage', { products: doc })
        })
    } else {
        res.render('admin') //ไม่มี session จะ render หน้าเข้าสู่ระบบขึ้นมา
    }
})

//ออกจากระบบ
router.get('/logout', (req, res) => {
    req.session.destroy((err) => { //destroy ตัว session ทิ้งไป
        res.redirect('/manage')
    })
})
...
router.post('/login', (req, res) => {
    ...
    if (username === "admin" && password === "123") {
        ...
        req.session.cookie.maxAge = timeExpire //เอา cookie ไปฝังไว้ทีุ่ web browser
        res.redirect('/manage')
    } else {
        res.render('404')
    }
